...
module.exports = {
  ...
  execute(message, args) {
    if (!args[0]) {
      ...
      descArgEmbed.setDescription(
        "Please provide a valid command to give a description of! You can find the commands by executing `>commands`"
      );
      descArgEmbed.setColor(0x3366ff);

      return message.channel.send(descArgEmbed).then((msg) =>
        msg.delete({
          timeout: 3000,
        })
      );
      ...
      if (args[0] === "kick") {
        ...
        descPrefixEmbed.setDescription(
      ...
      descArgInvalidEmbed.setDescription(
