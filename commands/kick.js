...
module.exports = {
  ...
  execute(message, args, approvedEmoji, disapprovedEmoji, errEmbed){
        ...
        kickPermissionEmbed.setDescription(`${disapprovedEmoji} You do not have permission to kick members!`)
        kickPermissionEmbed.setColor(0x3366ff)
        
        const kickArgsEmbed = new Discord.MessageEmbed();
        kickArgsEmbed.setDescription(`${disapprovedEmoji} Please mention a user to kick!`)
        ...
        if(targetUser){
            if(member){
                member.kick({reason: reason}).then(() =>{
                    ...
                    kickSuccessEmbed.setDescription(`${approvedEmoji} Successfully kicked ${targetUser}`)
