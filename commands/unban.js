...
module.exports = {
  ...
  execute(message, args){
        ...
        unbanLogEmbed.addFields(
            {name: "Member's ID", value: userID, inline: true},
            {name: "Moderator Responsible", value: message.author.user.username, inline: true},
           ...
           return message.channel.send(banPermissionEmbed).then(msg => msg.delete({timeout: 3000}));
            ...
            message.guild.fetchBans().then(bans=> {
                ...
                message.guild.members.unban(bUser.user).then(() =>{
                    ...
                    message.guild.channels.cache.find("769609262636335144").send(unbanLogEmbed)
                }).catch(err =>{
